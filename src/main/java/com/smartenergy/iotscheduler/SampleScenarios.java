...
/**
 * Sample flows using Reactive API for Kafka. To run a sample scenario:
 * <ol>
 * <li>Start Zookeeper and Kafka server
 * <li>Create Kafka topics {@link #TOPICS}
 * <li>Update {@link #BOOTSTRAP_SERVERS} and {@link #TOPICS} if required
 * <li>Run {@link SampleScenarios} {@link Scenario} as Java application (eg.
 * {@link SampleScenarios} KAFKA_SINK) with all dependent jars in the CLASSPATH
 * (eg. from IDE).
...
public class SampleScenarios {
	...
	protected enum Scenario {
		KAFKA_SINK, KAFKA_SINK_CHAIN, KAFKA_SOURCE, KAFKA_TRANSFORM, ATMOST_ONCE, FAN_OUT, PARTITION
	}

	/**
	 * This sample demonstrates the use of Kafka as a sink when messages are
	 * transferred from an external source to a Kafka topic. Unlimited (very large)
	 * blocking time and retries are used to handle broker failures. Source records
	 * are committed when sends succeed.
	 *
	 */
	public static class KafkaSink extends AbstractScenario {
		...
		public Flux<?> flux() {
			...
			return sender(senderOptions)
					...
					.doOnError(e -> log.error("Send failed, terminating.", e)).doOnNext(r -> {
	 ...
	 * This sample demonstrates the use of Kafka as a sink when messages are
	 * transferred from an external source to a Kafka topic. Unlimited (very large)
	 * blocking time and retries are used to handle broker failures. Each source
	 * record is transformed into multiple Kafka records and the result records are
	 * sent to Kafka using chained outbound sequences. Source records are committed
	 * when sends succeed.
	 *
	 */
	public static class KafkaSinkChain extends AbstractScenario {
		...
		public Flux<?> flux() {
			...
			Flux<Person> srcFlux = source().flux();
			return srcFlux
					.concatMap(p -> sender.createOutbound().send(Mono.just(new ProducerRecord<>(topic1, p.id(), p)))
							.send(Mono.just(new ProducerRecord<>(topic2, p.id(), p.upperCase()))).then()
	...
	/**
	 * This sample demonstrates the use of Kafka as a source when messages are
	 * transferred from a Kafka topic to an external sink. Kafka offsets are
	 * committed when records are successfully transferred. Unlimited retries on the
	 * source Kafka Flux ensure that the Kafka consumer is restarted if there are
	 * any exceptions while processing records.
	 */
	public static class KafkaSource extends AbstractScenario {
		...
		public Flux<?> flux() {
			return KafkaReceiver.create(receiverOptions(Collections.singletonList(topic)).commitInterval(Duration.ZERO))
					...
					.concatMap(m -> storeInDB(m.value()).thenEmpty(m.receiverOffset().commit())).retry()
					.doOnCancel(() -> close());
		}
	...
	/**
	 * This sample demonstrates a flow where messages are consumed from a Kafka
	 * topic, transformed and the results stored in another Kafka topic. Manual
	 * acknowledgement ensures that offsets from the source are committed only after
	 * they have been transferred to the destination. Acknowledged offsets are
	...
	public static class KafkaTransform extends AbstractScenario {
		...
		public Flux<?> flux() {
			KafkaSender<Integer, Person> sender = sender(senderOptions());
			return KafkaReceiver.create(receiverOptions(Collections.singleton(sourceTopic))).receive()
					.map(m -> SenderRecord.create(transform(m.value()), m.receiverOffset())).as(sender::send)
					.doOnNext(m -> m.correlationMetadata().acknowledge()).doOnCancel(() -> close());
		}
	...
	/**
	 * This sample demonstrates a flow with at-most once delivery. A topic with
	 * replication factor one combined with a producer with acks=0 and no retries
	 * ensures that messages that could not be sent to Kafka on the first attempt
	 * are dropped. On the consumer side, {@link KafkaReceiver#receiveAtmostOnce()}
	 * commits offsets before delivery to the application to ensure that if the
	 * consumer restarts, messages are not redelivered.
	 */
	public static class AtmostOnce extends AbstractScenario {
		...
		public SenderOptions<Integer, Person> senderOptions() {
			return super.senderOptions().producerProperty(ProducerConfig.ACKS_CONFIG, "0")
					.producerProperty(ProducerConfig.RETRIES_CONFIG, "0").stopOnError(false);
		}

		public Flux<?> flux() {
			KafkaSender<Integer, Person> sender = sender(senderOptions());
			return KafkaReceiver.create(receiverOptions(Collections.singleton(sourceTopic))).receiveAtmostOnce()
					.map(cr -> SenderRecord.create(transform(cr.value()), cr.offset())).as(sender::send)
					.doOnCancel(() -> close());
		}
	...
	/**
	 * This sample demonstrates the use of transactions to send data to multiple
	 * topic partitions, such that if one of the sends fails, the transaction is
	 * aborted and the data from uncommitted transactions are not visible to
	 * consumers configured with {@link ConsumerConfig#ISOLATION_LEVEL_CONFIG}
	...
	public static class TransactionalSend extends AbstractScenario {
		...
		@Override
		public SenderOptions<Integer, Person> senderOptions() {
			return super.senderOptions().producerProperty(ProducerConfig.ACKS_CONFIG, "all")
		...
		public Flux<?> flux() {
			...
			Flux<Person> srcFlux = source().flux();
			return sender.sendTransactionally(srcFlux.map(p -> records(p))).concatMap(r -> r)
					.doOnNext(r -> log.info("Sent record successfully {}", r))
					.doOnError(e -> log.error("Send failed, terminating.", e)).doOnCancel(() -> close());
		}
		...
		private Flux<SenderRecord<Integer, Person, Integer>> records(Person p) {
			...
			SenderRecord<Integer, Person, Integer> record1 = SenderRecord.create(destTopic1, null, null, p.id(), p1,
					p.id());
			Person p2 = new Person(p.id(), p.firstName(), p.lastName());
			p2.email(p.lastName().toLowerCase(Locale.ROOT) + "@reactor.io");
			SenderRecord<Integer, Person, Integer> record2 = SenderRecord.create(destTopic2, null, null, p.id(), p2,
					p.id());
	...
	/**
	 * This sample demonstrates an exactly/once transactional flow where messages
	 * are received from partitions of a source topic, transformed and sent to a
	 * destination topic using a transactional sender. Consumers with
	 * {@link ConsumerConfig#ISOLATION_LEVEL_CONFIG} <code>read_committed</code>
	 * will consume exactly one copy of each message from the destination topic
	 * after the messages are committed by the sender. Offsets for the source
	 * partitions are committed using the transactional sender to ensure that even
	 * if exceptions are encountered, the flow restarts from the last committed
	 * state.
	 *
	 */
	public static class KafkaExactlyOnce extends AbstractScenario {
		...
		public ReceiverOptions<Integer, Person> receiverOptions() {
			return super.receiverOptions().consumerProperty(ConsumerConfig.ISOLATION_LEVEL_CONFIG, "read_committed");
		}
		...
		public Flux<?> flux() {
			...
			return receiver.receiveExactlyOnce(sender.transactionManager()).concatMap(f -> sendAndCommit(f))
		...
		private Flux<SenderResult<Integer>> sendAndCommit(Flux<ConsumerRecord<Integer, Person>> flux) {
			return sender
					.send(flux.map(r -> SenderRecord.<Integer, Person, Integer>create(transform(r.value()), r.key())))
	...
	/**
	 * This sample demonstrates a flow where messages are consumed from a Kafka
	 * topic and processed by multiple streams with each transformed stream of
	 * messages stored in a separate Kafka topic.
	 *
	 */
	public static class FanOut extends AbstractScenario {
		...
		public Flux<?> flux() {
			...
			Flux<?> inFlux = KafkaReceiver.create(receiverOptions(Collections.singleton(sourceTopic))).receiveAutoAck()
					.concatMap(r -> r).doOnNext(m -> incoming.next(m.value()));
			Flux<SenderResult<Integer>> stream1 = sender
					.send(processor.publishOn(scheduler1).map(p -> SenderRecord.create(process1(p, true), p.id())));
			Flux<SenderResult<Integer>> stream2 = sender
					.send(processor.publishOn(scheduler2).map(p -> SenderRecord.create(process2(p, true), p.id())));
			...
			return Flux.merge(stream1, stream2).doOnSubscribe(s -> cancelRef.set(inFlux.subscribe())).doOnCancel(() -> {
	...
	/**
	 * This sample demonstrates a flow where messages are consumed from a Kafka
	 * topic, processed by multiple threads and the results stored in another Kafka
	 * topic. Messages are grouped by partition to guarantee ordering in message
	 * processing and commit operations. Messages from each partition are processed
	 * on a single thread.
	 */
	public static class PartitionProcessor extends AbstractScenario {
		...
		public Flux<?> flux() {
			...
			return KafkaReceiver.create(receiverOptions(Collections.singleton(topic)).commitInterval(Duration.ZERO))
					...
					.flatMap(partitionFlux -> partitionFlux.publishOn(scheduler)
							.map(r -> processRecord(partitionFlux.key(), r)).sample(Duration.ofMillis(5000))
		...
		public ReceiverOffset processRecord(TopicPartition topicPartition, ReceiverRecord<Integer, Person> message) {
			log.info("Processing record {} from partition {} in thread{}", message.value().id(), topicPartition,
	...
	public static class Person {
		...
		public boolean equals(Object other) {
			...
			if (id != p.id)
				return false;
			return stringEquals(firstName, p.firstName) && stringEquals(lastName, p.lastName)
	...
	static abstract class AbstractScenario {
		...
		public ReceiverOptions<Integer, Person> receiverOptions(Collection<String> topics) {
			return receiverOptions().addAssignListener(p -> log.info("Group {} partitions assigned {}", groupId, p))
